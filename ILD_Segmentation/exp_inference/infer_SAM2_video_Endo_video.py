...
parser.add_argument(
    '--png_save_dir',
    type=str,
    default="./results/vidoes/Endo/overlay_base",
...
parser.add_argument(
    '--imgs_path',
    type=str,
    default="./data/videos/Endo/SUN-SEG_preprocessed_relabel/imgs",
    help='imgs path',
)
parser.add_argument(
    '--gts_path',
    type=str,
    default="./data/videos/Endo/SUN-SEG_preprocessed_relabel/gts",
    help='gts path',
)
parser.add_argument(
    '--pred_save_dir',
    type=str,
    default="./results/vidoes/Endo/segs_base",
    help='segs path',
)
...
def resize_rgb_video(array, image_size):
    ...
    Parameters:
        array (np.ndarray): Input array of shape (3, d, h, w) where 3 is for RGB.
    ...
    for i in range(d):
        img_array = array[:, i, :, :].transpose(1, 2, 0)  # Convert to shape (h, w, 3)
        img_pil = Image.fromarray(img_array.astype(np.uint8))
        img_resized = img_pil.resize((image_size, image_size))
        img_resized_array = np.array(img_resized).transpose(2, 0, 1)  # Convert back to shape (3, image_size, image_size)
...
def mask2D_to_bbox(gt2D):
    ...
    # add perturbation to bounding box coordinates
    H, W = gt2D.shape
    bbox_shift = np.random.randint(1, 5, 1)[0]
...
def mask3D_to_bbox(gt3D):
    ...
    z_min = max(0, z_min)
    z_max = min(D-1, z_max+1)
    boxes3d = np.array([x_min, y_min, z_min, x_max, y_max, z_max])
    return boxes3d

@torch.inference_mode()
def infer_video(img_mp4_file):
    print(f'infering {img_mp4_file}')
    mp4_name = basename(img_mp4_file)
    #npz_data = np.load(img_npz_file, 'r', allow_pickle=True)
    #mp4_data = # Open the video file
    ...
    while True:
        ...
        # Convert the frame to a NumPy array (though it's already a NumPy array)
    ...
    gts = np.load(os.path.join(gts_path, mp4_name.replace('.mp4','.npz')), 'r', allow_pickle=True)['gts']
    if 255 in gts:
        gts = (gts != 0).astype(np.uint8)
    img_3D = np.array(frames)  # (D, H, W, 3)
    img_3D = np.transpose(img_3D, (3, 0, 1, 2))  # (3, D, H, W)
    print(img_3D.shape)
    print(gts.shape)
    ...
    for idx, lab in enumerate(ulabs, start=1):
        ...
        box_2d = mask2D_to_bbox(gt_roi[z_mid])
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            img_slice = img_3D[z_mid_orig] #3, h, w
    ...
    np.savez_compressed(join(pred_save_dir, mp4_name.replace('.mp4', '.npz')), segs=segs_3D)

    if save_overlay:
        npz_gts = np.load(join(gts_path, mp4_name.replace('.mp4', '.npz')), 'r', allow_pickle=True)
        gts = npz_gts['gts']
        ...
        plt.tight_layout()
        plt.savefig(join(png_save_dir, mp4_name.split(".")[0] + '.png'), dpi=300)
        plt.close()


if __name__ == '__main__':
    img_npz_files = sorted(glob(join(imgs_path, '*.mp4'), recursive=True))
